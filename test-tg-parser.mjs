...
section("Дата в накладной — не сумма");
eq(parseItemLine("21.08 кр френч"), { name: "кр френч", qty: null, sum: null }, "«21.08» не стало суммой 21");
eq(parseItemLine("кр френч 21.08 4800"), { name: "кр френч", qty: null, sum: 4800 }, "дата отброшена, сумма настоящая");
eq(parseItemLine("круассан 21.08.2026 14шт 12200"), { name: "круассан", qty: 14, sum: 12200 }, "полная дата отброшена");
eq(parseItemLine("Кола 1.5 6шт 3000"), { name: "Кола 1.5", qty: 6, sum: 3000 }, "«1.5» в названии не спутано с датой");
eq(parseItemLine("Кола 0.5 12шт 6000"), { name: "Кола 0.5", qty: 12, sum: 6000 }, "«0.5» тоже цело");
eq(parseItemLine("пончики 40шт 40.000"), { name: "пончики", qty: 40, sum: 40000 }, "«40.000» — тысячи, не дата");
eq(parseItemLine("круассан 14шт 12.200"), { name: "круассан", qty: 14, sum: 12200 }, "«12.200» — тысячи");
...
// ─── Дата перед точкой ─────────────────────────────────────────────
{
  // «09.09 Атакент сиропы 21 600» — дата прятала точку, и «Атакент»
  // уезжал в название товара
  const r = parseInvoiceMessage("09.09 Атакент сиропы 21 600", "2026-09-25");
  eq(r.branch, "Атакент", "точка после даты распознана");
  eq(r.date, "2026-09-09", "дата ушла в дату накладной");
  eq(r.items[0]?.name, "сиропы", "в названии товара — только товар");
  eq(r.items[0]?.sum, 21600, "сумма на месте");
  const k = parseInvoiceMessage("9.09 коктем сиропы 5 шт 21600", "2026-09-25");
  eq(k.branch, "Коктем", "короткая дата и точка с маленькой буквы");
  eq(k.items[0]?.qty, 5, "штуки на месте");
  // Число без точки после — не дата-префикс, строку не трогаем
  eq(parseInvoiceMessage("21.08 4800", "2026-09-25").branch, null, "дата без точки после — точки нет");
}
