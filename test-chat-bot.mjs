...

{
  ...
  // Род прилагательного — по дню недели: «обычной пятницы», «обычного четверга»
  const wd = new Date(`${Y}T00:00:00Z`).getUTCDay();
  const word = ["обычного воскресенья", "обычного понедельника", "обычного вторника", "обычной среды", "обычного четверга", "обычной пятницы", "обычной субботы"][wd];
  ok(r.includes(`касса 50 000 ₸ — ниже ${word}`), `против ${word}: ${r.split("\n")[0]}`);
...
section("Прогноз на сегодня в боте");

{
  const r = (await answerQuestion("сколько сделаем сегодня", deps))?.text || "";
  ok(/Прогноз на сегодня: ~|ещё рано/.test(r), `прогноз по форме дня: ${r.split("\n")[0]}`);
  const p = (await answerQuestion("прогноз на сегодня", deps))?.text || "";
  ok(!/умеет только сайт/.test(p), "«прогноз на сегодня» бот понимает сам");
}

section("«Как дела» в боте");

{
  const r = (await answerQuestion("как дела у абаи", deps))?.text || "";
  ok(/Прогноз Абая на сегодня|Вчера:/.test(r), `темп дня или вчерашний разбор: ${r.split("\n")[0]}`);
}
