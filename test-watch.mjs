...

{
  ...
  ok(lines.length <= 8, `сообщение коротко: ${lines.length} строк вместо 16`);
  ok(t.includes("Милана — 4 чека, старший"), "чеки одного бариста собраны в строку");
  eq((t.match(/Милана/g) || []).length, 1, "бариста упомянут один раз, а не четырежды");
  ok(t.includes("8 000 ₸"), "суммы группы сложены");
  ok(t.includes("11 чеков открыты пустыми"), "пустые свёрнуты в счётчик");
  ok(!t.includes("Тома-Бибэк"), "пустые поимённо не перечисляются");
  ok(t.includes("Коктем — 59 мин"), "тишина на месте");
}

{
  // Когда зависших с деньгами много, показываем верхушку и остаток суммой
  const rows = [];
  for (let i = 0; i < 10; i++) rows.push(open(`x${i}`, String((i % 8) + 1), 100 + i * 10, 100000, "Б" + i));
  const t = formatAlerts(buildAlerts(rows, { now: NOW })).replace(/<[^>]+>/g, "").replace(/\u00A0/g, " ");
  ok(/и ещё \d+ — [\d ]+ ₸/.test(t), "остаток свёрнут в строку с суммой");
  ok(t.split("\n").filter((l) => l.startsWith("•")).length <= 7, "строк не больше семи");
}
...
{
  // Только пустые: сумм нет, но знать надо
  const t = formatAlerts(buildAlerts([open("1", "4", 30, 0), open("2", "4", 40, 0)], { now: NOW }));
  ok(t.includes("2 чека открыты пустыми"), "счётчик по-русски");
  ok(!t.includes("Чеки висят открытыми"), "заголовка про деньги нет, когда денег нет");
}
