...

{
  ...
  for (const q of ["привет", "спасибо", "ок", "да", "ладно", "что нового"]) {
...
section("Живая проверка 26.09.2026: месяц внутри слова, «в мае», «день недели»");

{
  // «сАМАЯ» содержит «мая» — отвечало кассой за май
  const w = await ask("самая слабая точка за месяц");
  ok(w.period.from.slice(5, 7) !== "05", `«самая» — не май: ${w.period.from}`);
  eq(w.operation, "min", "«самая слабая» — худшая точка");
  eq((await ask("касса в мае")).period.from.slice(5, 7), "05", "«в мае» — май (раньше не понималось вовсе)");
  const d = await ask("лучший день недели");
  ok(d.period.from < d.period.to && d.period.to < new Date().toLocaleDateString("sv-SE"), "«лучший день недели» — четыре полные недели, а не текущая");
  const why = await ask("почему просел жароково вчера");
  eq(why.assumed.metric, false, "у «почему» касса — по смыслу, без приписки «показал кассу»");
  eq((await ask("как дела у абаи")).status, true, "«как дела» — сводка «как идём»");
}

section("Формулировки «как человек спросит» (прогон 26.09.2026)");

{
  const cases = [
    ["средний чек растёт?", "avgCheck", "percentChange"],
    ["кто больше всех продал в этом месяце", "staff", "max"],
    ["сколько мы должны заплатить налогов", "tax", null],
    ["что заканчивается на складе", "stock", null],
    ["минусы на складе", "stock", null],
    ["план на месяц", "forecast", "forecast"],
    ["сколько сделаем в этом месяце", "forecast", "forecast"],
    ["у кого самый большой средний чек", "avgCheck", "max"],
  ];
  for (const [q, metric, op] of cases) {
    const p = await ask(q);
    eq(op ? [p?.metric, p?.operation] : p?.metric, op ? [metric, op] : metric, `«${q}»`);
    ok(!p?.product || !/заплатить|этом|план|сделаем|заканчива/.test(p.product), `«${q}»: служебное слово не стало товаром`);
  }
  eq((await ask("что с коктемом")).status, true, "«что с Коктемом» — как дела у точки");
  eq((await ask("что лучше продаётся латте или капучино")).products2, ["латте", "капучино"], "два товара через «или» — сравнение, а не второй из них");
  ...
  }
  {
    ...
    ok(d2.getDay() === 5 && d2 < d1 && (d1 - d2) / 86400000 <= 7, `против прошлой пятницы: ${v.period2.from}`);
  }
}
...

{
  ...
  // «Как дела» у владельца — про кассу сегодня, а не приветствие
  const kd = await ask("как дела");
  eq([kd?.metric, kd?.period?.from, kd?.product], ["cash", "2026-09-20", null], "«как дела» — касса за сегодня");
...

{
  ...
  // Сменилась тема в уточнении — операция прошлого вопроса ей чужая:
  // «что скоро закончится» после «сколько сделаем сегодня» отвечало
  // прогнозом кассы (26.09.2026)
  {
    const prev = await ask("сколько сделаем сегодня");
    const next = await mergeFollowUp(prev, "что скоро закончится");
    eq([next?.metric, next?.operation], ["stock", "sum"], "тема — остатки, операция — не «прогноз»");
  }
...

{
  ...
  // Разговорное
  eq((await ask("как торгуем")).period.from, "2026-09-20", "«как торгуем» — сегодня");
  eq((await ask("выручка щас")).period.from, "2026-09-20", "«щас» — сегодня");
  eq((await ask("сколько наторговали вчера")).metric, "cash", "«наторговали» — касса");
  const lim = await ask("10 худших товаров");
  eq([lim.metric, lim.operation, lim.limit], ["products", "min", 10], "«10 худших» — минимум, десять строк");
  eq((await ask("покажи топ 5 товаров")).limit, 5, "«топ 5» — пять строк");
  eq((await ask("сколько кофе на абае")).product, "кофе", "«на абае» не прилипает к товару");
  const vk = await ask("выручка кофе");
  eq([vk.metric, vk.product], ["products", "кофе"], "«выручка кофе» — продажи кофе");
  eq((await ask("сколько мы должны налогов")).metric, "tax", "«должны налогов» — налоги, не товар «должны»");
  eq((await ask("средний чек вырос?")).operation, "percentChange", "«средний чек вырос» — рост, не среднее");
  eq((await ask("средняя касса за неделю")).operation, "average", "а «средняя касса» — среднее");
  eq([(await ask("касса летом")).period.from, (await ask("касса летом")).period.to], ["2026-06-01", "2026-08-31"], "летом — июнь–август");
  eq([(await ask("касса зимой")).period.from, (await ask("касса зимой")).period.to], ["2025-12-01", "2026-02-28"], "зимой — прошлая зима на стыке годов");
  eq((await ask("касса осенью")).period.to, "2026-09-20", "осенью — по сегодня");
  eq((await ask("касса первую половину сентября")).product, null, "«первую» — не товар");
  eq((await ask("самый дорогой чек за неделю")).product, null, "«дорогой» — не товар");
  // Дни в прошлом и «в это же время»
  eq((await ask("что было в этот день год назад")).period.from, "2025-09-20", "этот день год назад");
  eq((await ask("что было в этот день год назад")).metric, "cash", "и это касса, а не разрез по дням");
  eq([(await ask("касса в прошлом сентябре")).period.from, (await ask("касса в прошлом сентябре")).period.to], ["2025-09-01", "2025-09-30"], "прошлый сентябрь");
  eq((await ask("как прошлый вторник")).period.from, "2026-09-08", "«как прошлый вторник» — вторник прошлой недели");
  const sy = await ask("сравни сегодня со вчера в это же время");
  eq([sy.period.from, sy.period2?.from, sy.hours?.from], ["2026-09-20", "2026-09-19", 0], "сегодня против вчера в том же окне часов");
  eq((await ask("сколько будет к концу месяца")).metric, "forecast", "«к концу месяца» — прогноз");
  eq((await ask("касса по дням недели за месяц")).period.from, "2026-09-01", "«по дням недели за месяц» — месяц, а не неделя");
  const bw = await ask("сравни выходные с буднями");
  eq([bw.metric, bw.operation], ["cash", "byWeekday"], "«сравни выходные с буднями» — разрез по дням недели");
  // Себестоимость и наценка
  eq([(await ask("себестоимость латте")).metric, (await ask("себестоимость латте")).product], ["margin", "латте"], "себестоимость позиции");
  eq((await ask("какая наценка на круассан")).product, "круассан", "наценка на позицию");
  eq((await ask("сколько стоит латте")).metric, "margin", "«сколько стоит» — про цену, а не продажи");
  eq((await ask("сколько латте продали")).metric, "products", "а «сколько продали» — продажи");
  // Закрытие, состав смены, скидки
  eq((await ask("во сколько закрылись вчера")).metric, "opening", "закрытие — та же метрика, что открытие");
  eq((await ask("во сколько закрывались точки за неделю")).metric, "opening", "и «закрывались» тоже");
  eq((await ask("до скольки работала абая вчера")).spot.posterName, "Abaya", "«до скольки» — про точку");
  eq((await ask("кто работал вчера на абае")).metric, "staff", "«кто работал» — состав смены");
  eq((await ask("сколько скидок дали за неделю")).metric, "discounts", "скидки");
  eq((await ask("сколько скидок дали за неделю")).product, null, "и «скидок» не товар");
  eq((await ask("сколько смен отработала айгерим")).person, "айгерим", "имя — последним словом");
  // Две точки рядом — сравнение товаров
  const vs = await ask("что на абае берут чаще чем на дубае");
  eq([vs.metric, vs.spot.posterName, vs.spot2?.posterName], ["products", "Abaya", "Dubai"], "первая названная — та, про которую спрашивают");
  eq((await ask("сравни товары дубай и абая")).spot.posterName, "Dubai", "порядок сохраняется");
  eq((await ask("сравни абая и дубай")).metric, "compareBranches", "без слова «товары» — сравнение кассы точек");
  eq((await ask("товары абая")).spot2, undefined, "одна точка — без второй");
  // Бариста
...
section("Оценка — не название товара, но размер в названии остаётся");

{
  // «Самые большие чеки вчера» отвечало «товар „большие" не найден»:
  // прилагательное уходило в название, метрика становилась товарами, и
  // до разбора крупных чеков дело не доходило.
  const big = await ask("самые большие чеки вчера");
  ok(!big?.product, `товар не выдуман: ${big?.product}`);
  eq(big?.metric, "checks", "«самые большие чеки» — про чеки");
  eq(big?.operation, "max", "и это поиск максимума");

  for (const q of ["самый крупный чек за неделю", "максимальный чек вчера", "самый дорогой чек вчера"]) {
    const p = await ask(q);
    eq(p?.metric, "checks", `«${q}» — чеки`);
    eq(p?.operation, "max", `«${q}» — максимум`);
  }
