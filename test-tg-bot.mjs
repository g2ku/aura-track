...
{
  // Так кураторы подписывают фото бумажных накладных: дата, точка, товар
  // — без суммы. Бот фото не читает, и раньше такая накладная пропадала
  // молча: дата впереди прятала точку, а без точки и суммы — «return null»
  const store = makeStore({ ackMode: "reply" });
  const r = await run(store, "09.09 Атакент сиропы", { asPhoto: true });
  ok(r && /фото накладной я не читаю/.test(r.text), `фото без суммы — сказано прямо: ${r?.text?.split("\n")[0]}`);
  ok(/Атакент/.test(r.text), "точка распознана, хотя подпись начинается с даты");
  ok(/Приход не записан/.test(r.text), "и что приход не записан");
  ok(/09\.09 Атакент сиропы 21600/.test(r.text), "подсказка — та же подпись с суммой");
  eq((await store.getDoc(TODAY))?.totals || {}, {}, "в базу ничего не записано");

  // Та же подпись с суммой — принята, в свою дату и в свою точку
  ...
  // А обычная болтовня текстом по-прежнему без ответа
  eq(await run(makeStore({ ackMode: "reply" }), "сиропы завтра привезут"), null, "текстовая болтовня — молчим");
}
...
section("/анализ: за период не записано ничего — сказано почему");
{
  // На проде «/анализ месяц сиропы» отвечал «ничего похожего не
  // приходило» — хотя сиропы приходили, но фотографиями без суммы.
  // Звучало как «не привозили», а на деле бот не записал ничего вообще
  const store = makeStore({ admins: [999] });
  const r = await run(store, "/анализ месяц сиропы", { userId: 999 });
  ok(r && /бот не записал ни одной накладной/.test(r.text), `пустой период назван прямо: ${r?.text?.split("\n")[0]}`);
  ok(/Фото накладных бот не читает/.test(r.text), "и названа частая причина");
  ok(/09\.09 Атакент сиропы 21600/.test(r.text), "с примером, как писать");

  // Если накладные были, но не такие — прежний ответ со списком
  await run(store, "Абая\nПончики 48шт 40000");
