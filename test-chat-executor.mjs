...
import { summarizeBaristas, summarizeDeleted } from "./api/_lib/baristas.js";
...
globalThis.__poster = {
  ...
  async fetchIngredientMovement(from, to) {
    const items = [
      { id: "milk", name: "Молоко Обычное 2,5%", unit: "l", price: 663, spent: 90, money: 59670, negativeAt: ["OBI"],
        byBranch: { "Абая": { spent: 120, income: 0, end: 20 }, "OBI": { spent: 30, income: 0, end: -12 } } },
  ...
  async fetchBaristas(from, to) {
    ...
    // Удалённые — строками dash со status 3, как отдаёт Poster: каждый
    // день один на Абае вечером; 18-го — ещё один, уже оплаченный, на Дубае
    const deletedRows = [];
    for (const d of eachDay(from, to).filter((x) => x <= TODAY)) {
      deletedRows.push({ status: "3", transaction_id: `del-${d}`, spot_id: "4", name: "Данияр", sum: 320000, payed_sum: 0,
        date_close: new RealDate(`${d}T21:40:00+05:00`).getTime() });
      if (d === "2026-09-18") deletedRows.push({ status: "3", transaction_id: "del-paid", spot_id: "9", name: "Айгерим", sum: 150000, payed_sum: 150000,
        date_close: new RealDate(`${d}T15:10:00+05:00`).getTime() });
    }
    return { ...summarizeBaristas(rows), deleted: summarizeDeleted([...rows, ...deletedRows]) };
  },
  _receipts(from, to) {
...
section("«Почему» — разбор причин: люди или покупки, часы, товары");
{
  ...
  ok(/^Абая, 19 сентября 2026 г\.: касса [\d  ]+ ₸ — (ниже|выше|в пределах) обычной субботы/.test(t), `первая строка — против обычной субботы (род!): ${t.split("\n")[0]}`);
...
section("«Как дела», «самая слабая точка»");
{
  const st = plain(await ask("как дела у абаи"));
  ok(/Прогноз Абая на сегодня|Сегодня продаж ещё нет|Сегодня пока/.test(st), `«как дела» — темп дня или вчерашний разбор: ${st.split("\n")[0]}`);
  const weak = plain(await ask("самая слабая точка за неделю"));
  ok(/^Слабее всех за .+ — \S+: [\d  ]+ ₸, \d+ % кассы сети/.test(weak), `сначала ответ: ${weak.split("\n")[0]}`);
}

section("Самый большой средний чек — ответ первой строкой");
{
  const t = plain(await ask("у кого самый большой средний чек за неделю"));
  ok(/^Самый большой средний чек за .+ — \S+: [\d  ]+ ₸ \(по сети/.test(t), `сначала ответ: ${t.split("\n")[0]}`);
  const low = plain(await ask("что заканчивается на складе"));
  ok(/Скоро закончится|ничего не кончится/.test(low), "«заканчивается» — вопрос о запасе");
}

section("Два товара рядом: «латте или круассан»");
{
  const t = plain(await ask("что лучше продаётся латте или круассан за неделю"));
  ok(/^Латте берут в 2,0 раза чаще, чем круассан/.test(t), `ответ первой строкой: ${t.split("\n")[0]}`);
  ok(/• Латте: \d+ шт \/ [\d  ]+ ₸/.test(t) && /• Круассан: \d+ шт/.test(t), "штуки и деньги по каждому");
}
...
section("Живые формулировки: еда, выпечка, удалённые чеки, выходные, лучший день");
{
  // Разделы меню — по справочнику Poster, а не товар с таким словом
  const bake = await ask("продажи выпечки за неделю");
  has(bake, "Выпечка за", "«выпечки» — раздел «Выпечка»");
  has(bake, "Круассан", "с товарами раздела");
  ok(!plain(bake).includes("Латте"), "без кофе");
  const food = await ask("продажи еды за неделю");
  has(food, "Еда (Выпечка) за", "«еда» — съедобные разделы, и сказано какие");
  ok(!plain(food).includes("Латте"), "кофе — не еда");
  const none = await ask("десерты за неделю");
  has(none, "В меню Poster нет раздела «десерты»", "раздела нет — так и сказано, а не «товар не найден»");
  has(none, "Разделы меню: Кофе, Выпечка, Зимнее меню", "и какие разделы есть");

  // Удалённые чеки — из той же сводки сервера, что бариста
  const del = await ask("удалённые чеки вчера");
  has(del, "Удалённые чеки все филиалы за 19 сентября 2026 г.: 1 чек на 3 200 ₸", "сколько и на какую сумму");
  has(del, "• Абая: 1 чек на 3 200 ₸ — Данияр 1", "вся сеть — по точкам, с людьми");
  has(await ask("удалённые чеки на Абае вчера"), "Кто удалял: Данияр 1", "одна точка — кто удалял");
  has(del, "19.09 21:40 Абая — 3 200 ₸, Данияр", "сам чек со временем и точкой");
  const del3 = await ask("удалённые чеки за 3 дня");
  has(del3, "4 чека на 11 100 ₸", "за три дня — четыре чека");
  has(del3, "Из них один был оплачен: 1 500 ₸", "оплаченный удалённый — отдельным предупреждением");
  has(del3, "• Абая: 3 чека на 9 600 ₸ — Данияр 3", "по точкам — с людьми");
  const delD = await ask("удалённые чеки на Дубае вчера");
  has(delD, "Удалённых чеков Дубай за 19 сентября 2026 г. нет", "нет — так и сказано");

  // «Как прошли выходные» в воскресенье: суббота против прошлой субботы
  const we = await ask("как прошли выходные");
  has(we, "Выходные против выходных неделей раньше.", "сказано, что сравниваем");
  has(we, "Сегодня не считал — день ещё идёт", "неполное воскресенье не сравнивается");
  has(we, "12 сентября 2026 г. (1 дн.) → 19 сентября 2026 г. (1 дн.)", "суббота к субботе, а не к пятнице");

  // Лучший и худший день — даты. Фикстура: день ×1,2 при числе, дающем 4
  // по модулю 5 — 04, 09, 14, 19; ×0,8 — 05, 10, 15 (20-е — сегодня, не в счёт)
  const best = await ask("лучший день в сентябре");
  has(best, "Лучшие дни по кассе все филиалы за сентябрь 2026 г.:", "заголовок");
  has(best, "(504 чека)", "чеки — с верным окончанием");
  has(best, "🏆 Пт 04.09 — 1 200 000 ₸", "лучшая дата");
  has(best, "04.09 больше всех принесла Абая — 600 000 ₸", "и кто сделал этот день");
  has(best, "В среднем сильнее всего —", "дни недели — одной строкой внизу");
  const worst = await ask("худший день месяца");
  has(worst, "📉 Сб 05.09 — 800 000 ₸", "худшая дата");
  ok(!plain(worst).includes("20.09"), "сегодняшний неполный день в худшие не попал");
  const top5 = await ask("топ 5 дней за сентябрь");
  ok((plain(top5).match(/\n(?:🏆|🥈|🥉|•) /g) || []).length === 5, "топ-5 — пять строк");
}
