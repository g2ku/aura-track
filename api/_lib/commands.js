...
async function handleCommand({ cmd, args }, ctx) {
  ...
  switch (cmd) {
    ...
    case "ask": {
      ...
      if (!q) {
        return { text: "Спросите словами:\n<code>/спроси касса вчера</code>\n<code>/спроси чеки Абая за неделю</code>\n<code>/спроси что продавалось лучше всего</code>\n<code>/спроси сравни август и сентябрь</code>\n<code>/спроси кто просел за неделю</code>\n<code>/спроси способы оплаты вчера</code>\n<code>/спроси тренд кассы</code>\n<code>/спроси касса по будням за месяц</code>\n\nВ личке можно и без команды — просто напишите вопрос. Под ответом — кнопки со следующими вопросами." };
      }
...
export async function handleMessage(msg, ctx) {
  ...
  if (!branch) {
    // Фото с подписью, но без точки и без суммы — «сиропы». В чате
    // накладных это почти наверняка накладная, и молчание её теряет
    if (!hasItems && (msg.photo || msg.document)) {
      return {
        text: [
          "📷 Фото накладной я не читаю — приход не записан.",
          "",
          "Напишите подписью точку и сумму:",
          "<code>09.09 Атакент сиропы 21600</code>",
        ].join("\n"),
      };
    }
  ...
  if (!hasItems) {
    // Фото накладной с подписью без суммы — «09.09 Атакент сиропы». Бот
    // фото не читает, и такая накладная раньше пропадала молча: присылал
    // думал, что приход записан, а в учёт не попадало ничего. Говорим
    // прямо и показываем, что дописать. Для фото — даже в теме точки:
    // фото с подписью в чате накладных почти всегда накладная.
    const isPhoto = !!(msg.photo || msg.document);
    if (isPhoto) {
      const cap = escapeHtml(text.trim().split(/\n/)[0].slice(0, 60));
      return {
        text: [
          `📷 <b>${escapeHtml(branch)}</b> — фото накладной я не читаю, сумму взять неоткуда. Приход не записан.`,
          "",
          "Допишите сумму в подпись (можно и штуки):",
          `<code>${cap} 21600</code> — вместо 21600 ваша сумма`,
        ].join("\n"),
      };
    }
