...

{
  ...
  eq(seen.length, 23, "владелец видит все пункты (без выдуманных «Авто-остатков», 25.09.2026)");
...

{
  ...
  eq(seen("curator").filter((id) => ["payroll", "admin-users", "pnl", "margin", "taxes"].includes(id)), [],
     "куратору не видно ни денег сети, ни налогов, ни настроек");
  // Налоги по ИП вернулись в меню: маршрут пропал 13 августа в чужом коммите
  ok(seen("admin").includes("taxes") && seen("manager").includes("taxes"), "налоги по ИП — владельцу и управляющему");
  ok(!seen("admin").includes("replenish"), "«Авто-остатки» с выдуманным остатком в меню не вернулись");
}
