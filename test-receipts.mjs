...

{
  ...
  ok(/receipts-table \.data-table > tbody > tr > td:nth-child\(1\):not\(\.receipt-details\)/.test(css),
     "строка с составом исключена из скрытия первой колонки");

  // Правила скрытия — только для прямых ячеек внешней таблицы. Без «>»
  // они задевали таблицу состава внутри раскрытого чека и прятали её
  // первую колонку: на телефоне было «1 · 150 000 ₸» без названия товара
  const block = css.slice(css.indexOf("Чеки на телефоне"), css.indexOf("Инвентаризация: скрываем"));
  ok(block.length > 0, "правила чеков для телефона на месте");
  ok(!/receipts-table t[dh]:nth-child/.test(block), "ни одного правила без «>» — состав чека не задевается");

  // На телефоне видно время закрытия, а статус (дубль фильтра) спрятан
  const hides = (n) => new RegExp(`> t[dh]:nth-child\\(${n}\\)`).test(block);
  ok(!hides(5), "«Закрыт» (время) на телефоне видна");
  ok(hides(9), "«Статус» спрятан — сверху есть фильтр открытых и закрытых");
  ok(/receipts-table \.data-table \{ min-width: 0; \}/.test(block), "четыре колонки не растягиваются общим минимумом 500 px");
  ok(/td\.receipt-details td:first-child \{ white-space: normal; \}/.test(block), "длинное название товара в составе переносится");
