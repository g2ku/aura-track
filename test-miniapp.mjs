...
section("Отвергнутая досылка не исчезает молча");

{
  const app = readFileSync("src/miniapp/App.jsx", "utf8");
  ok(/const bad = r\.failed\.filter\(\(f\) => f\.error\)/.test(app) && /setDropped\(/.test(app), "что сервер отверг при досылке — запоминается");
  ok(app.includes("Не записалось — сервер отказал, введите заново"), "и показывается человеку, а не только уменьшает счётчик");
  ok(/onClick=\{\(\) => setDropped\(\[\]\)\}/.test(app), "с кнопкой «Понятно»");
}
...

{
  ...
  // Весь прогноз держится на отметке «было до приезда», а какая точка её
  // ждёт — до сих пор не говорилось нигде: why считался и пропадал
  ok(html.includes("нужен ещё один пересчёт"), "у точки без прогноза сказано, чего не хватает");

  const noCount = render(h(Warehouse, {
    state, skus: SKUS, branches: ["Абая", "Рамс"], today: [],
    forecast: [{ branch: "Рамс", daysLeft: null, why: "не пересчитывали", left: null }],
    onSend: noop, isAdmin: true,
  }));
  ok(noCount.includes("отметьте «было до приезда»"), "и сказано, что именно сделать");
  ok(!noCount.includes("расход пока не посчитался"), "чужой причины на экране нет");
}
