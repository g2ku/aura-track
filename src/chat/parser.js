...
const METRICS = [
  ...
  { keys: ["бариста", "сотрудник", "официант", "по людям", "кто из ребят", "кто продал больше", "кто пробил больше", "по кассирам", "кассир", "кто работал", "кто стоял", "кто был на смене", "кто сегодня работает", "чья смена", "смен отработал"], value: "staff" },
  ...
  { keys: ["расход", "остатк", "остаток", "списан", "ингредиент", "минус по", "в минусе", "сколько ушло", "сколько потрачен", "скоро закончится", "что закончится", "на сколько хватит", "хватит ли"], value: "stock" },
...
export async function parseQuestion(text) {
  ...
  const willAsk = /(?:сколько|какая|какой)\s+(?:сделаем|сделает|будет|выйдет|набер[её]м|заработаем)|к\s+(?:закрытию|концу\s+дня)|до\s+конца\s+дня/.test(lower) && !/(?:^|[^а-яё])(?:месяц|недел|квартал|год)/.test(lower);
