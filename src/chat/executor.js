...
//
// «Почему просел Жароково вчера» — не голое «−12 %», а из чего оно
// сложилось: меньше людей (чеков) или меньше покупали (средний чек), в
...
async function handleWhy(spot, period, ipGroup) {
  ...
  if (period.from >= todayIso) {
    return { text: `Сегодня день ещё идёт — причины видно по закончившемуся дню. Спросите: «почему ${isAll(spot) ? "просела касса" : `просел ${sl}`} вчера».`, data: null };
  }
  // «За неделю» с сегодняшним неполным днём — разбираем полные дни
  if (period.to >= todayIso) period = { ...period, to: shift(todayIso, -1) };
  const days = daysInPeriod(period.from, period.to);
  const oneDay = days === 1;
  const bases = oneDay
    ? [7, 14, 21, 28].map((k) => ({ from: shift(period.from, -k), to: shift(period.from, -k) }))
    : [{ from: shift(period.from, -days), to: shift(period.from, -1) }];
  const [cur, ...baseData] = await Promise.all([
