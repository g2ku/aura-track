...
{
  // Часы считаются ПО ДНЯМ. Раньше брался интервал за весь период, и у
  // бариста со 157 чеками за неделю выходил «1 чек в час»: делилось на
  // 150 часов календаря вместе с ночами.
  const d1 = Date.parse("2026-08-26T04:00:00Z");
  const d2 = Date.parse("2026-08-27T04:00:00Z");
  const rows = [
    tx("4", "81", "Сабина", 1000, d1), tx("4", "81", "Сабина", 1000, d1 + 4 * H),
    tx("4", "81", "Сабина", 1000, d2), tx("4", "81", "Сабина", 1000, d2 + 4 * H),
  ];
  const { people } = summarizeBaristas(rows);
  eq(people[0].hours, 8, "восемь часов за два дня, а не сутки между ними");
  eq(people[0].daysWorked, 2, "отработано два дня");
  eq(people[0].perHour, 0.5, "четыре чека на восемь часов");
  eq(summarizeBaristas([]).people, [], "пустой ответ не роняет");
  eq(summarizeBaristas(null).people, [], "и null тоже");
}

{
  // Настоящий случай: у Адията один чек, а показывалось «2 чека в час».
  // Минимум в полчаса, поставленный от деления на ноль, выдумывал цифру.
  const rows = [tx("10", "5", "Адият", 1300, Date.now())];
  const p = summarizeBaristas(rows).people[0];
  eq(p.checks, 1, "чек один");
  eq(p.perHour, null, "по одному чеку скорость не показываем");
  eq(p.daysWorked, 1, "но день отработан");
}

{
  // Меньше часа за прилавком — тоже прочерк
  const at = Date.parse("2026-08-26T04:00:00Z");
  const p = summarizeBaristas([
    tx("4", "9", "Быстрый", 500, at), tx("4", "9", "Быстрый", 500, at + 20 * 60000),
  ]).people[0];
  eq(p.perHour, null, "двадцать минут — не смена");
}
