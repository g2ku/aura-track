...
section("«Сравнить точки»: период до сегодня — к тому же часу");

{
  // 26.09.2026: «Сегодня» против полного вчера давало «−40 %» у всех к обеду
  const src = (await import("node:fs")).readFileSync("src/components/CrossLocationDashboard.jsx", "utf8");
  ok(/dateTo === todayStr\(\)[\s\S]{0,200}fetchHoursByDay\(prevToStr, prevToStr\)/.test(src), "последний день прошлого периода режется по часам");
  ok(/c\.total - full \+ part/.test(src), "полный день заменяется частью до того же часа");
  ok(/к тому же часу прошлого периода/.test(src), "и на экране сказано, с чем сравнение");
}

section("Главная: «вчера к этому часу» и «вчера за день» — разными строками");

{
  // 26.09.2026: «Вчера за день 2 128 760 ₸ +7 %» при кассе 33 тыс. — процент
  // был к вчерашнему дню на этот час, а стоял рядом с итогом всего дня
